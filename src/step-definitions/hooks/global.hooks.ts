...
export const config = {
  browser:
    process.env.UI_AUTOMATION_BROWSER ||
    envConfig.parsed?.UI_AUTOMATION_BROWSER ||
    "chromium",
  headless:
    process.env.HEADLESS === "true" || envConfig.parsed?.HEADLESS === "true",
  browserWidth: parseInt(
    process.env.BROWSER_WIDTH || envConfig.parsed?.BROWSER_WIDTH || "1920"
  ),
  browserHeight: parseInt(
    process.env.BROWSER_HEIGHT || envConfig.parsed?.BROWSER_HEIGHT || "1080"
  ),
};
...
After(async function ({ pickle, result }) {
  ...
  if (result?.status === Status.FAILED && pageFixture.page) {
    try {
      const { image, path } = await takeScreenshot(
        pageFixture.page,
        pickle.name
      );
